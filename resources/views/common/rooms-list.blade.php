<x-app>
    <main>
        <div class="container-fluid bg-trasparent my-4 p-3" style="position: relative;">
            <button class="btn btn-success" type="button" data-toggle="collapse" data-target="#collapse-filters" aria-expanded="false" aria-controls="collapse-filters">
                Filters
            </button>
            ...
            <div id="rooms-list" class="row row-cols-1 row-cols-sm-1 row-cols-md-4 row-cols-bg-6 g-4">
                @foreach($rooms as $room)
                    <div
                        class="col mb-4"
                        data-name="{{ $room->name }}"
                        data-city="{{ $room->establishment->city }}"
